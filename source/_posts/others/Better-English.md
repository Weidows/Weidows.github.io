...
|        trigger        | n. 扳机；起因，引起反应的事；`触发器`，引爆装置, v. 触发，引起                                           |
|         i18n          | （其来源是英文单词 internationalization 的首末字符 i 和 n，18 为中间的字符数）是“国际化”的简称。         |
|         grid          | n. 网格；格子，栅格；输电网                                                                              |
|        router         | n. [计] 路由器；刳刨者                                                                                   |
|       protocol        | n. 礼仪，礼节；国际议定书，`协议`；条约草案                                                              |
|        dynamic        | 动态的                                                                                                   |
...
|         bind          | vi. `结合`；装订；有约束力；过紧, vt. 绑；约束；装订；包扎；凝固, n. 捆绑；困境；                        |
...
|       allocate        | v. 分配，分派，划拨                                                                                      |
|        primary        | adj. 主要的，首要的；<英>小学教育的，初级教育的；初级的，原发性的；最基本的，最根本的                    |
|       definite        | 一定的                                                                                                   |
|       capacity        | 容量                                                                                                     |
|         given         | 已知的                                                                                                   |
|       algorithm       | 算法                                                                                                     |
...
|  underscore   | \_   |         comma          | ,     |
|    dollar     | $    |       back-quote       | `     |
|    percent    | %    |      single-quote      | '     |
|     tilde     | ~    |      double-quote      | "     |
|      at       | @    | left/right-parenthesis | ( / ) |
|     hash      | #    |    left/right-brace    | { / } |
|  dot/period   | .    |   left/right-bracket   | [ / ] |
| forward-slash | /    |    left/right-angle    | < / > |
| star/asterisk | \*   |         caret          | ^     |
|     plus      | +    |       back-slash       | \     |
|  dash/minus   | -    |     forward-slash      | /     |
|     equal     | =    |     question-mark      | ?     |
