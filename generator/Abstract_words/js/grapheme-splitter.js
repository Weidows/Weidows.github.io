...
function GraphemeSplitter(){
	...
	function isSurrogate(str, pos) {return  0xd800 <= str.charCodeAt(pos) && str.charCodeAt(pos) <= 0xdbff &&
	...
	function codePointAt(str, idx){if(idx === undefined){idx = 0;}
		...
		if (0xDC00 <= code && code <= 0xDFFF &&
			idx >= 1){var hi = str.charCodeAt(idx - 1);
			var low = code;
	...
	function shouldBreak(start, mid, end){var all = [start].concat(mid).concat([end]);
		...
		if(eModifierIndex> 1 &&
			...
			[Extend, E_Base, E_Base_GAZ].indexOf(start) == -1){return Break}
		...
		if(rIIndex> 0 &&
			...
			[Prepend, Regional_Indicator].indexOf(previous) == -1) {if(all.filter(function(c){return c == Regional_Indicator}).length % 2 == 1) {return BreakLastRegional}
		...
		// GB6. L X (L|V|LV|LVT)
			...
			(next == L || next == V || next == LV || next == LVT)){return NotBreak;}
		// GB7. (LV|V) X (V|T)
		else if((previous == LV || previous == V) &&
			(next == V || next == T)){return NotBreak;}
		// GB8. (LVT|T) X (T)
		else if((previous == LVT || previous == T) &&
			next == T){return NotBreak;}
		// GB9. X (Extend|ZWJ)
	...
	this.splitGraphemes = function(str){var res = [];
		var index = 0;
		var brk;
		while((brk = this.nextBreak(str, index)) <str.length){res.push(str.slice(index, brk));
			index = brk;
		}
	...
	this.iterateGraphemes = function(str) {
		...
		var res = {next: (function() {
				var value;
				var brk;
				if ((brk = this.nextBreak(str, index)) <str.length) {value = str.slice(index, brk);
					index = brk;
					return {value: value, done: false};
				}
				if (index < str.length) {value = str.slice(index);
					index = str.length;
					return {value: value, done: false};
				}
	...
	function getGraphemeBreakProperty(code){
		...
		if(0x200D == code // Cf       ZERO WIDTH JOINER){return ZWJ;}
